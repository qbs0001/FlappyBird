...
class GameScene: SKScene, SKPhysicsContactDelegate /* 追加 */ {
    ...
    let noneCategory: UInt32 = 0 // 0...00000
    // スコア用
    var score = 0 // ←追加
    ...
    func setupGround() {
        ...
        // 必要な枚数を計算
        let needNumber = Int(frame.size.width / groundTexture.size().width) + 2
    ...
    func setupCloud() {
        ...
        // 必要な枚数を計算
        let needCloudNumber = Int(frame.size.width / cloudTexture.size().width) + 2
    ...
    func setupWall() {
        ...
        // 移動する距離を計算
        let movingDistance = CGFloat(frame.size.width + wallTexture.size().width)
        ...
        let center_y = groundSize.height + (frame.size.height - groundSize.height) / 2
        ...
        let createWallAnimation = SKAction.run {
            ...
            // アイテムの出現位置を上壁と下壁を中心にランダム（−５０〜５０）に設定する
            let randomItem_y = CGFloat.random(in: -50..<50)
            item.position = CGPoint(x: 0, y: ((under_wall_y + upper.position.y) / 2) + randomItem_y)

            // スプライトに物理演算を設定する
            item.physicsBody = SKPhysicsBody(circleOfRadius: item.frame.width * 0.2)
            // アイテムカテゴリとする
    ...
    func setupBird() {
        ...
        bird.position = CGPoint(x: frame.size.width * 0.2, y: frame.size.height * 0.7)
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & itemCategory) == itemCategory || (contact.bodyB.categoryBitMask & itemCategory) == itemCategory {
            ...
            // 効果音を出した後に、ノードを削除する
            item.node!.run(SKAction.sequence([playSound, SKAction.removeFromParent()]))

            // ノードのカテゴリをゼロにして、重複カウントを抑止する
            item.categoryBitMask = noneCategory
            item.contactTestBitMask = noneCategory
        }

        // 壁か地面用
        if (contact.bodyA.categoryBitMask & wallCategory) == wallCategory || (contact.bodyB.categoryBitMask & wallCategory) == wallCategory || (contact.bodyA.categoryBitMask & groundCategory) == groundCategory || (contact.bodyB.categoryBitMask & groundCategory) == groundCategory {
            // 壁か地面と衝突した
            print("GameOver")
    ...
    func setupScoreLabel() {
        ...
        itemScoreLabelNode.position = CGPoint(x: 10, y: frame.size.height - 120)
        ...
        bestItemScoreLabelNode.position = CGPoint(x: 10, y: frame.size.height - 150)
        ...
        bestItemScoreLabelNode.text = "Best Item Score:\(bestItemScore)"
        addChild(bestItemScoreLabelNode)
    }
}
